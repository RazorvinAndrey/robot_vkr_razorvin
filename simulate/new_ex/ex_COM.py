...
class Robot:
    ...
    def move(self):
        if self.step_index < len(self.steps) and not self.stop:
            ...
            proposed_x = self.x + distance * np.cos(rad_angle) + random.uniform(-0.15, 0.15)
            proposed_y = self.y + distance * np.sin(rad_angle) + random.uniform(-0.15, 0.15)
    ...
    def avoid_collision(self):
        for angle_offset in [40, -40, 70, -70, 100, -100, 130, -130, 180]:
            rad_angle = np.deg2rad(self.angle + angle_offset)
            proposed_x = self.x + 0.2 * np.cos(rad_angle)
            proposed_y = self.y + 0.2 * np.sin(rad_angle)
            if not self.check_collision(proposed_x, proposed_y, self.robots):
                self.x, self.y = proposed_x, proposed_y
                self.path.append((self.x, self.y))
                self.steps = self.calculate_path_steps(self.target_position)
    ...
    def measure_distance(self):
        ...
        for angle_step in range(-10, 11):  # Проверяем углы от -5 до 5 градусов относительно текущего направления робота для учета небольших расхождений
