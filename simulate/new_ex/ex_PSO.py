...
class PSORobot:
    ...
    def update_velocity(self, global_best_position, delta_time=1):
        if not self.at_target:
            ...
            velocity = inertia * self.velocity + cognitive_velocity + social_velocity + np.random.rand(2) * 0.5
    ...
    def update_angle(self, delta_time):
        if np.linalg.norm(self.velocity) > 0:  # Убеждаемся, что скорость не нулевая
            ...
            angle_diff = (target_angle - self.angle + np.pi) % (2 * np.pi) - np.pi + random.uniform(-np.pi/12, np.pi/12)
    ...
    def avoid_obstacles_and_robots(self):
        ...
        buffer = 0.2  # Буферное расстояние для избежания столкновений
    ...
    def find_alternative_path(self):
        ...
        for angle in np.linspace(-np.pi, np.pi, 6, endpoint=True):
    ...
    def avoid_obstacle(self):
        ...
        for angle_offset in [np.pi / 4, -np.pi / 4, np.pi / 2, -np.pi / 2, np.pi / 1.2, -np.pi / 1.2]:
    ...
    def animate(self):
        def update(frame):
            ...
            for robot in self.robots:
                ...
                self.ax.plot(robot.position[0], robot.position[1], 'ro')
                self.ax.plot(robot.target[0], robot.target[1], 'bs')
